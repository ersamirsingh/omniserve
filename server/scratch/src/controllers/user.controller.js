...
export class UserController {
     ...
     */
    static async acceptMyInvitation(req, res) {
        try {
    ...
    static async createUser(req, res) {
        try {
            ...
            if (!RoleHierarchy.canManageRole(req.user.role, role)) {
            ...
            const user = await UserService.createUser(req.user.tenantId, userData, req.user.userId, req.user.role, req.user.restaurantId, req.user.outletId);
    ...
    static async updateUser(req, res) {
        try {
            ...
            const hasManagerPrivileges = [UserRole.SUPER_ADMIN, UserRole.RESTAURANT_OWNER].includes(req.user.role);
            ...
            if (role !== undefined && !isSelfUpdate && !RoleHierarchy.canManageRole(req.user.role, role)) {
            ...
            const user = await UserService.updateUser(id, req.user.tenantId, updateData, req.user.userId, req.user.role, req.user.restaurantId, req.user.outletId);
            if (!user) {
                ApiResponseHandler.notFound(res, 'User not found');
                return;
            }
    ...
    static async deleteUser(req, res) {
        try {
            ...
            const deletedUser = await UserService.deleteUser(id, req.user.tenantId, req.user.userId, req.user.role);
            if (!deletedUser) {
                ApiResponseHandler.notFound(res, 'User not found');
                return;
            }
