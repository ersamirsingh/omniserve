...
export class OrderController {
    ...
    static async placeOrder(req, res) {
        try {
            ...
            const { outletId, customerId, source, subtotal, tax, deliveryFee, discount, totalAmount, notes, items, } = req.body;
    ...
    static async listOrders(req, res) {
        try {
            ...
            const scopedOrders = allowedOutletIds === null || outletId
                ? orders
                : orders.filter(order => {
                    const oid = order.outletId?._id?.toString() || order.outletId.toString();
    ...
    static async getOrderById(req, res) {
        try {
            ...
            const outletIdStr = details.order.outletId?._id?.toString() || details.order.outletId?.toString();
    ...
    static async updateOrderStatus(req, res) {
        try {
            ...
            const order = await OrderService.updateOrderStatus(id, req.user.tenantId, orderStatus, req.user.userId);
            if (!order) {
                ApiResponseHandler.notFound(res, 'Order not found');
                return;
            }
            ...
            if (orderStatus === OrderStatus.ACCEPTED)
                responseData.acceptedAt = order.acceptedAt;
            if (orderStatus === OrderStatus.PREPARING)
                responseData.preparedAt = order.preparedAt;
            if (orderStatus === OrderStatus.READY)
                responseData.readyAt = order.readyAt;
            if (orderStatus === OrderStatus.PICKED_UP)
                responseData.pickedUpAt = order.pickedUpAt;
            if (orderStatus === OrderStatus.DELIVERED)
                responseData.deliveredAt = order.deliveredAt;
    ...
    static async cancelOrder(req, res) {
        try {
            ...
            const order = await OrderService.cancelOrder(id, req.user.tenantId, cancellationReason.trim(), req.user.userId);
            if (!order) {
                ApiResponseHandler.notFound(res, 'Order not found');
                return;
            }
    ...
    static async addItemToOrder(req, res) {
        try {
            ...
            const orderItem = await OrderService.addItemToOrder(id, req.user.tenantId, itemData, req.user.userId);
            if (!orderItem) {
                ApiResponseHandler.notFound(res, 'Order not found');
                return;
            }
