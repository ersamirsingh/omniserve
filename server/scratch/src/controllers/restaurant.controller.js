...
export class RestaurantController {
    static async getRestaurants(req, res) {
        try {
            ...
            if (req.user?.role !== UserRole.SUPER_ADMIN && req.user?.restaurantId) {
                restaurants = restaurants.filter((restaurant) => restaurant._id.toString() === req.user?.restaurantId);
            }
    ...
    static async createRestaurant(req, res) {
        try {
            ...
            const restaurant = await RestaurantService.createRestaurant(tenantId, name, description, brandName, gstNumber, logoUrl);
            res.status(200).json({
                sucess: true,
    ...
    static async updateRestaurant(req, res) {
        try {
            ...
            const restaurant = await RestaurantService.updateRestaurant(tenantId, restaurantId, { name, description, brandName, gstNumber, logoUrl });
            if (!restaurant) {
                res.status(404).json({ message: 'Restaurant not found' });
                return;
            }
