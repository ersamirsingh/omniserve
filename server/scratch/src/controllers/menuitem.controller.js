...
export class MenuItemController {
    ...
    static async createMenuItem(req, res) {
        try {
            ...
            const { categoryId, outletId, name, price, description, image, sku, isVeg, isAvailable, displayOrder, } = req.body;
            ...
            const menuItem = await MenuItemService.createMenuItem(req.user.tenantId, menuItemData, req.user.userId);
    ...
    static async updateMenuItem(req, res) {
        try {
            ...
            const updatedItem = await MenuItemService.updateMenuItemDetails(id, req.user.tenantId, updateData, req.user.userId);
            if (!updatedItem) {
                ApiResponseHandler.notFound(res, 'Menu item not found');
                return;
            }
    ...
    static async toggleAvailability(req, res) {
        try {
            ...
            const updatedItem = await MenuItemService.updateAvailabilityStatus(id, req.user.tenantId, !!isAvailable, req.user.userId);
            if (!updatedItem) {
                ApiResponseHandler.notFound(res, 'Menu item not found');
                return;
            }
    ...
    static async deleteMenuItem(req, res) {
        try {
            ...
            const deletedItem = await MenuItemService.deleteMenuItem(id, req.user.tenantId, req.user.userId);
            if (!deletedItem) {
                ApiResponseHandler.notFound(res, 'Menu item not found');
                return;
            }
