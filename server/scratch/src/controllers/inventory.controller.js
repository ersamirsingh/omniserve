...
export class InventoryController {
    ...
    static async createInventory(req, res) {
        try {
            ...
            const inventory = await InventoryService.createInventory(req.user.tenantId, inventoryData, req.user.userId);
    ...
    static async listInventory(req, res) {
        try {
            ...
            const { inventory, total } = await InventoryService.getInventory(req.user.tenantId, filters);
            const scopedInventory = allowedOutletIds === null || outletId
                ? inventory
                : inventory.filter(item => {
                    const oid = item.outletId?._id?.toString() || item.outletId.toString();
    ...
    static async updateQuantity(req, res) {
        try {
            ...
            const updatedInventory = await InventoryService.updateQuantity(id, req.user.tenantId, numQuantity, req.user.userId);
            if (!updatedInventory) {
                ApiResponseHandler.notFound(res, 'Inventory record not found');
                return;
            }
     ...
     */
    static async listLowStock(req, res) {
        try {
            if (!req.user?.tenantId) {
                ApiResponseHandler.unauthorized(res, 'User not authenticated or tenantId not found');
                return;
            ...
            if (outletId)
                filters.outletId = outletId;
            const { inventory, total } = await InventoryService.getLowStockInventory(req.user.tenantId, filters);
            const scopedInventory = allowedOutletIds === null || outletId
                ? inventory
