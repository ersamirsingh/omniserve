...
const integrationEventQueueSchema = new Schema({
    tenantId: {
    ...
    aggregateType: {
        ...
        enum: ["ORDER", "MENU_ITEM", "INVENTORY", "CART", "TABLE", "WAITER_TASK"],
        trim: true,
    },
    aggregateId: {
        type: Schema.Types.ObjectId,
...
integrationEventQueueSchema.index({ tenantId: 1, eventType: 1, aggregateId: 1, correlationId: 1, eventVersion: 1 }, { unique: true });
const IntegrationEventQueue = mongoose.model("IntegrationEventQueue", integrationEventQueueSchema);
