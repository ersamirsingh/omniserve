...
export class CustomerService {
     ...
     */
    static async checkConflicts(tenantId, phone, email, excludeCustomerId) {
        const tenantObjectId = new Types.ObjectId(tenantId);
        // Check phone conflict
        const phoneConflictQuery = {
     ...
     */
    static async upsertCustomer(tenantId, data, userId) {
        const tenantObjectId = new Types.ObjectId(tenantId);
     ...
     */
    static async getCustomers(tenantId, filters) {
        const query = {
     ...
     */
    static async getCustomerById(id, tenantId) {
     ...
     */
    static async updateCustomer(id, tenantId, data, userId) {
     ...
     */
    static async deleteCustomer(id, tenantId, userId) {
        /**
     ...
     */
    static async addAddress(customerId, tenantId, addressData, userId) {
        const customer = await Customer.findOne({
     ...
     */
    static async updateAddress(customerId, tenantId, addrId, addressData, userId) {
        const customer = await Customer.findOne({
            _id: new Types.ObjectId(customerId),
            tenantId: new Types.ObjectId(tenantId),
            isDeleted: false,
        ...
        const addressSubdoc = customer.address.id(addrId);
        if (!addressSubdoc) {
            return null;
        }
        ...
        if (isBecomingDefault && !addressSubdoc.isDefault) {
            ...
            const otherAddr = customer.address.find((addr) => addr._id.toString() !== addrId);
            if (otherAddr) {
        ...
        if (addressData.label !== undefined)
            addressSubdoc.label = addressData.label;
        if (addressData.line1 !== undefined)
            addressSubdoc.line1 = addressData.line1;
        if (addressData.line2 !== undefined)
            addressSubdoc.line2 = addressData.line2;
        if (addressData.city !== undefined)
            addressSubdoc.city = addressData.city;
        if (addressData.state !== undefined)
            addressSubdoc.state = addressData.state;
        if (addressData.pincode !== undefined)
            addressSubdoc.pincode = addressData.pincode;
        if (addressData.location !== undefined)
            addressSubdoc.location = addressData.location;
        customer.updatedBy = userId ? new Types.ObjectId(userId) : null;
        const savedCustomer = await customer.save();
        return savedCustomer.address.id(addrId) || null;
    }
    /**
     * Delete an Address from Customer (supports auto-promoting fallback default address)
     */
    static async deleteAddress(customerId, tenantId, addrId, userId) {
        const customer = await Customer.findOne({
            _id: new Types.ObjectId(customerId),
            tenantId: new Types.ObjectId(tenantId),
            isDeleted: false,
