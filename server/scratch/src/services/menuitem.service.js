...
export class MenuItemService {
     ...
     */
    static async validateHierarchy(categoryId, outletId, tenantId) {
        try {
     ...
     */
    static async createMenuItem(tenantId, data, userId) {
        ...
        await EventBusService.publishMenuChanged(tenantId, saved.outletId, saved._id, "MENU_ITEM", saved, { createdBy: userId }).catch(err => console.error('Failed to publish MENU_CHANGED event:', err));
        return saved;
    }
    /**
     * List menu items with filters and text/regex search
     */
    static async getMenuItems(tenantId, filters) {
        const query = {
            tenantId: new Types.ObjectId(tenantId),
            isDeleted: false,
        };
     ...
     */
    static async getMenuItemWithDetails(id, tenantId) {
    ...
    static async getMenuItemById(id, tenantId) {
     ...
     */
    static async updateMenuItemDetails(id, tenantId, data, userId) {
        ...
        if (updated) {
            await EventBusService.publishMenuChanged(tenantId, updated.outletId, updated._id, "MENU_ITEM", updated, { createdBy: userId }).catch(err => console.error('Failed to publish MENU_CHANGED event (update):', err));
        }
        return updated;
    }
    /**
     ...
     */
    static async updateAvailabilityStatus(id, tenantId, isAvailable, userId) {
        const updated = await MenuItem.findOneAndUpdate({
            _id: new Types.ObjectId(id),
            tenantId: new Types.ObjectId(tenantId),
            isDeleted: false,
        ...
        if (updated) {
            await EventBusService.publishMenuChanged(tenantId, updated.outletId, updated._id, "MENU_ITEM", updated, { createdBy: userId }).catch(err => console.error('Failed to publish MENU_CHANGED event (toggle):', err));
        }
        return updated;
    }
    /**
     ...
     */
    static async deleteMenuItem(id, tenantId, userId) {
        ...
        if (deletedMenuItem) {
            await EventBusService.publishMenuChanged(tenantId, deletedMenuItem.outletId, deletedMenuItem._id, "MENU_ITEM", deletedMenuItem, { createdBy: userId }).catch(err => console.error('Failed to publish MENU_CHANGED event (delete):', err));
        }
