...
export class WebhookService {
     ...
     */
    static async resolveTenantAndSecret(provider, payload, headers, query) {
     ...
     */
    static validateSignature(provider, rawBody, signature, secret) {
        if (!signature || !secret)
            return false;
        try {
     ...
     */
    static async logWebhook(tenantId, provider, eventType, payload, signature, externalEventId, status, errorMessage = null, httpStatusCode = null) {
     ...
     */
    static async processWebhook(logId) {
        ...
        try {
            ...
            await WebhookLog.updateOne({ _id: log._id }, { status: WebhookStatus.PROCESSING });
            ...
            if (provider === WebhookProvider.STRIPE || provider === WebhookProvider.RAZORPAY) {
                ...
                if (orderId && Types.ObjectId.isValid(orderId)) {
                    ...
                    if (paymentStatus) {
                        ...
                        await Order.updateOne({ _id: new Types.ObjectId(orderId), tenantId: log.tenantId }, { paymentStatus });
                        await Payment.updateOne({ orderId: new Types.ObjectId(orderId), tenantId: log.tenantId }, { status: paymentStatus });
                    }
                }
            }
            // 2. Swiggy, Zomato, Dunzo, Porter (Order / Delivery Lifecycle Updates)
            if (provider === WebhookProvider.SWIGGY ||
            ...
            // Mark as SUCCESS
            await WebhookLog.updateOne({ _id: log._id }, {
     ...
     */
    static async retryWebhook(logId, tenantId) {
