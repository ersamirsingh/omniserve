...
import { PaymentStatus, NotificationType } from '../enums/enums.js';
import { NotificationService } from './notification.service.js';
export class PaymentService {
    /**
     * Process/Create a new payment for an order
     */
    static async createPayment(tenantId, data, userId) {
        ...
        try {
            ...
            await Order.updateOne({ _id: orderObjectId, tenantId: tenantObjectId }, {
            ...
            NotificationService.notifyTenantUsers(tenantId, title, message, nType, savedPayment._id.toString(), 'Payment', userId).catch(err => console.error('Failed to dispatch payment notification:', err));
            return savedPayment;
        }
        catch (error) {
     ...
     */
    static async refundPayment(id, tenantId, refundTransactionId, userId) {
        const tenantObjectId = new Types.ObjectId(tenantId);
        ...
        session.startTransaction();
        try {
            const updatedPayment = await Payment.findOneAndUpdate({ _id: paymentObjectId, tenantId: tenantObjectId }, {
            ...
            await Order.updateOne({ _id: payment.orderId, tenantId: tenantObjectId }, {
     ...
     */
    static async getPaymentById(id, tenantId) {
     ...
     */
    static async getPaymentByOrderId(orderId, tenantId) {
        return await Payment.findOne({
