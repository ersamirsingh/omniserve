...
export class OrderService {
     ...
     */
    static async validateOrderHierarchy(tenantId, outletId, customerId, items) {
     ...
     */
    static async placeOrder(tenantId, data, userId) {
        const { outletId, customerId, source, subtotal, tax = 0, deliveryFee = 0, discount = 0, totalAmount, notes, items, } = data;
        ...
        try {
            ...
            // Publish event
            await EventBusService.publishOrderCreated(tenantId, savedOrder.outletId, savedOrder._id, savedOrder, {
            ...
            NotificationService.notifyTenantUsers(tenantId, 'New Order Placed', `Order ${savedOrder.orderNumber} has been placed. Total amount: ${savedOrder.totalAmount}.`, NotificationType.ORDER_PLACED, savedOrder._id.toString(), 'Order', userId).catch(err => console.error('Failed to dispatch ORDER_PLACED notification:', err));
            return savedOrder;
        }
        catch (error) {
     ...
     */
    static async getOrders(tenantId, filters) {
        const query = {
            tenantId: new Types.ObjectId(tenantId),
            isDeleted: false,
        };
     ...
     */
    static async getOrderWithDetails(id, tenantId) {
     ...
     */
    static async updateOrderStatus(id, tenantId, newStatus, userId) {
        const order = await Order.findOne({
            _id: new Types.ObjectId(id),
            tenantId: new Types.ObjectId(tenantId),
            isDeleted: false,
        ...
        try {
            ...
            if (newStatus === OrderStatus.ACCEPTED) {
                ...
                for (const update of stockUpdates) {
                    await Inventory.updateOne({ _id: update.inventory._id }, {
                ...
                if (currentStatus !== OrderStatus.DELIVERED) {
                    await Customer.updateOne({ _id: order.customerId, tenantId: order.tenantId }, { $inc: { totalOrders: 1, totalSpent: order.totalAmount } }, { session });
                }
            }
            const updatedOrder = await Order.findOneAndUpdate({ _id: order._id, tenantId: order.tenantId }, updateFields, { new: true, session });
            await OrderTimeline.create([{
                    tenantId: order.tenantId,
                    orderId: order._id,
                    status: newStatus,
            ...
            if (updatedOrder) {
                // Publish event
                await EventBusService.publishOrderStatusChanged(tenantId, updatedOrder.outletId, updatedOrder._id, updatedOrder, {
                ...
                if (nType) {
                    NotificationService.notifyTenantUsers(tenantId, title, message, nType, updatedOrder._id.toString(), 'Order', userId).catch(err => console.error('Failed to dispatch status update notification:', err));
                }
            }
            return updatedOrder;
        }
     ...
     */
    static async cancelOrder(id, tenantId, cancellationReason, userId) {
        const order = await Order.findOne({
            _id: new Types.ObjectId(id),
            tenantId: new Types.ObjectId(tenantId),
            isDeleted: false,
        ...
        try {
            ...
            const updatedOrder = await Order.findOneAndUpdate({ _id: order._id, tenantId: order.tenantId }, {
            ...
            if (updatedOrder) {
                ...
                NotificationService.notifyTenantUsers(tenantId, 'Order Cancelled', `Order ${updatedOrder.orderNumber} has been cancelled. Reason: ${cancellationReason}`, NotificationType.ORDER_CANCELLED, updatedOrder._id.toString(), 'Order', userId).catch(err => console.error('Failed to dispatch ORDER_CANCELLED notification:', err));
            }
            return updatedOrder;
        }
        catch (error) {
     ...
     */
    static async getOrderItems(orderId, tenantId) {
     ...
     */
    static async addItemToOrder(orderId, tenantId, itemData, userId) {
        const order = await Order.findOne({
        ...
        try {
            const orderItem = new OrderItem({
                ...
                addons: itemData.addons ? itemData.addons.map((a) => ({
                    addonId: new Types.ObjectId(a.addonId),
                    name: a.name,
                    price: a.price,
                })) : [],
            ...
            await Order.updateOne({ _id: order._id }, { subtotal, totalAmount, updatedBy: userId ? new Types.ObjectId(userId) : null }, { session });
            await session.commitTransaction();
            return savedItem;
        }
        catch (error) {
     ...
     */
    static async deleteOrder(id, tenantId, userId) {
        const session = await mongoose.startSession();
        session.startTransaction();
        try {
            ...
            await OrderItem.updateMany({ orderId: order._id, tenantId: new Types.ObjectId(tenantId) }, { isDeleted: true, updatedBy: userId ? new Types.ObjectId(userId) : null }, { session });
            await session.commitTransaction();
            return order;
        }
        catch (error) {
