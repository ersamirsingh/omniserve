...
export class TokenBlacklistService {
    ...
    static async addToBlacklist(token, expiresIn) {
        try {
            ...
            await this.redisClient.setEx(`blacklist:${token}`, expiresIn, JSON.stringify({
     ...
     */
    static async trackUserToken(userId, token, expiresIn) {
        try {
            await this.init();
            // Create a unique key for this token under the user
            const tokenKey = `user_tokens:${userId}:${Date.now()}`;
            await this.redisClient.setEx(tokenKey, expiresIn, JSON.stringify({
