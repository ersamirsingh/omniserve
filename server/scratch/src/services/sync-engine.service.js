...
export class WorkerRegistry {
    registry = new Map();
    register(eventType, worker) {
        this.registry.set(eventType, worker);
        console.log(`[WorkerRegistry] Registered worker for event type: ${eventType}`);
    }
    get(eventType) {
...
export class SyncEngineService {
     ...
     */
    static async isCircuitOpen(tenantId, outletId, provider) {
        if (!outletId)
            return false;
     ...
     */
    static async handleSuccess(tenantId, outletId, provider) {
        if (!outletId)
            return;
        await ProviderSyncState.findOneAndUpdate({ tenantId, outletId, provider }, {
            $set: {
     ...
     */
    static async handleFailure(tenantId, outletId, provider) {
        if (!outletId)
            return;
        const state = await ProviderSyncState.findOne({ tenantId, outletId, provider });
     ...
     */
    static async processEvent(event, nodeId = "node-default") {
        ...
        try {
            ...
            console.log(`[SyncEngineService] Event ${event._id} processed successfully. Latencies: QueueWait=${queueWaitTime}ms, Processing=${processingTime}ms, E2E=${e2eTime}ms`);
        }
        catch (error) {
