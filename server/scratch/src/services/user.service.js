...
export class UserService {
     ...
     */
    static async checkEmailConflict(email, excludeUserId) {
        const query = {
    ...
    static async validateAssignment(tenantId, data, actorRole, actorRestaurantId, actorOutletId, excludeUserId) {
        ...
        const ids = [];
        if (data.outletId)
            ids.push(data.outletId.toString());
        if (data.outletIds && Array.isArray(data.outletIds)) {
            ids.push(...data.outletIds.map((id) => id.toString()));
        }
        if (ids.length > 0) {
        ...
        await this.ensureScopedRoleAvailable(tenantId, role, data.restaurantId, ids, excludeUserId);
    }
    static async ensureScopedRoleAvailable(tenantId, role, restaurantId, outletIdOrIds, excludeUserId) {
        ...
        if (role === UserRole.RESTAURANT_OWNER) {
            ...
            if (ids.length === 0)
                return;
            const outletObjectIds = ids.map((id) => new Types.ObjectId(id.toString()));
            query.$or = [
    ...
    static async sendInvitationEmailAndNotification(user, requestedRole, password, inviteLink, restaurantId, outletId, inviterUserId) {
      ...
      `,
        ...
        await NotificationService.createNotification(user.tenantId.toString(), user._id.toString(), 'Invitation pending', `Accept your ${requestedRole} invitation for ${restaurantName}${outletId ? ` / ${outletName}` : ''}.`, NotificationType.SYSTEM, user._id.toString(), 'UserInvitation', inviterUserId);
    }
    /**
     * Create a new User under a specific tenant
     */
    static async createUser(tenantId, data, creatorUserId, creatorRole, creatorRestaurantId, creatorOutletId) {
        const emailClean = data.email.trim().toLowerCase();
        await this.checkEmailConflict(emailClean);
        if (creatorRole && !RoleHierarchy.canManageRole(creatorRole, data.role)) {
        ...
        let primaryOutletId = null;
        const outletObjectIds = [];
        if (data.outletId) {
        ...
        if (isInvitation) {
            await this.sendInvitationEmailAndNotification(savedUser, data.role, data.password, invitationLink || this.buildInviteLink(''), data.restaurantId, primaryOutletId?.toString(), creatorUserId);
        }
        return savedUser;
    }
    static async acceptInvitation(userId, tenantId) {
        ...
        await this.validateAssignment(tenantId, {
            ...
            outletIds: user.pendingOutletIds?.map((id) => id.toString()),
     ...
     */
    static async updateUser(id, tenantId, data, updaterUserId, updaterRole, updaterRestaurantId, updaterOutletId) {
        const user = await User.findOne({
            _id: new Types.ObjectId(id),
            tenantId: new Types.ObjectId(tenantId),
            isDeleted: false,
        ...
        if (!isSelfUpdate && updaterRole && !RoleHierarchy.canManageRole(updaterRole, user.role)) {
        ...
        if (data.role !== undefined) {
            ...
            if (!isSelfUpdate && updaterRole && !RoleHierarchy.canManageRole(updaterRole, data.role)) {
        ...
        if (data.restaurantId !== undefined || data.outletId !== undefined || data.outletIds !== undefined || data.role !== undefined) {
            if (isSelfUpdate) {
                ...
                if (data.outletIds !== undefined) {
                    const newIds = data.outletIds.map((x) => x.toString()).sort().join(',');
            ...
            let outletIds = data.outletIds !== undefined
                ? data.outletIds.map((id) => id.toString())
            ...
            user.outletIds = assignmentData.outletIds ? assignmentData.outletIds.map((id) => new Types.ObjectId(id)) : [];
        }
     ...
     */
    static async deleteUser(id, tenantId, deleterUserId, deleterRole) {
        if (id === deleterUserId) {
        ...
        if (deleterRole && !RoleHierarchy.canManageRole(deleterRole, user.role)) {
