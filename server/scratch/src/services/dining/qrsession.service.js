...
export class QRSessionService {
     ...
     */
    static async createSession(tenantId, outletId, tableId, options = {}) {
     ...
     */
    static async getActiveSessionByTable(tableId) {
        const table = await Table.findOne({ _id: new Types.ObjectId(tableId), isDeleted: false });
     ...
     */
    static async joinSessionByCode(joinCode, customerId, seatNumber, options = {}) {
     ...
     */
    static async updateSessionStatus(sessionId, status, options = {}) {
        ...
        if (tableStatus) {
            ...
            if (status === "CLOSED") {
                await Table.findOneAndUpdate({ _id: session.tableId }, {
                    $set: {
            ...
            await TableService.updateTableOperationalStatus(session.tenantId, session.outletId, session.tableId, tableStatus, updateOpts);
        }
        return session;
    }
    /**
     ...
     */
    static async assignWaiter(sessionId, waiterId) {
        const session = await QRSession.findOneAndUpdate({ _id: new Types.ObjectId(sessionId), isDeleted: false }, { $set: { waiterId: new Types.ObjectId(waiterId) } }, { new: true });
        if (!session) {
            throw new Error(`Session not found: ${sessionId}`);
        }
        return session;
