...
export class WaiterTaskService {
     ...
     */
    static async getSlaLimitForTask(outletId, taskType) {
        try {
     ...
     */
    static async createTask(tenantId, outletId, tableId, sessionId, taskType, source, options = {}) {
        ...
        await EventBusService.publishWaiterTaskCreated(tenantId, outletId, task._id, {
     ...
     */
    static async assignTask(taskId, waiterId) {
        const task = await WaiterTask.findOneAndUpdate({ _id: new Types.ObjectId(taskId), status: "CREATED", isDeleted: false }, {
            $set: {
                status: "ASSIGNED",
        ...
        // Publish event
        await EventBusService.publishWaiterTaskAssigned(task.tenantId, task.outletId, task._id, {
            taskId: task._id.toString(),
     ...
     */
    static async acknowledgeTask(taskId, waiterId) {
        const task = await WaiterTask.findOneAndUpdate({ _id: new Types.ObjectId(taskId), status: { $in: ["CREATED", "ASSIGNED", "ESCALATED"] }, isDeleted: false }, {
            $set: {
                status: "ACKNOWLEDGED",
                assignedWaiterId: new Types.ObjectId(waiterId),
        ...
        // Publish event
        await EventBusService.publishWaiterTaskAcknowledged(task.tenantId, task.outletId, task._id, {
            taskId: task._id.toString(),
            waiterId: waiterId.toString(),
     ...
     */
    static async startTaskProgress(taskId) {
        const task = await WaiterTask.findOneAndUpdate({ _id: new Types.ObjectId(taskId), status: "ACKNOWLEDGED", isDeleted: false }, {
            $set: {
                status: "IN_PROGRESS",
        ...
        // Publish event
        await EventBusService.publishWaiterTaskInProgress(task.tenantId, task.outletId, task._id, {
            taskId: task._id.toString(),
    ...
    static async completeTask(taskId) {
        const task = await WaiterTask.findOneAndUpdate({ _id: new Types.ObjectId(taskId), status: { $in: ["CREATED", "ASSIGNED", "ACKNOWLEDGED", "IN_PROGRESS", "ESCALATED"] }, isDeleted: false }, {
            $set: {
                status: "COMPLETED",
        ...
        // Publish event
        await EventBusService.publishWaiterTaskCompleted(task.tenantId, task.outletId, task._id, {
            taskId: task._id.toString(),
    ...
    static async cancelTask(taskId, reason) {
        ...
        const updatedTask = await WaiterTask.findOneAndUpdate({ _id: new Types.ObjectId(taskId), status: { $in: ["CREATED", "ASSIGNED", "ACKNOWLEDGED", "IN_PROGRESS", "ESCALATED"] }, isDeleted: false }, {
            $set: {
                status: "CANCELLED",
        ...
        // Publish event
        await EventBusService.publishWaiterTaskCancelled(updatedTask.tenantId, updatedTask.outletId, updatedTask._id, {
     ...
     */
    static async getPendingTasksByOutlet(tenantId, outletId) {
