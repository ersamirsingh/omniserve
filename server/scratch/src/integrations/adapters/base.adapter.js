...
export class BaseAdapter {
    async verifySignature(args) {
        if (!args.secret)
            return true; // Bypass signature if no secret configured (local testing)
    ...
    mapStatusToProvider(internalStatus) {
    ...
    async buildInventoryPayload(args) {
        return {
            provider: this.provider,
            outletId: args.outletId,
