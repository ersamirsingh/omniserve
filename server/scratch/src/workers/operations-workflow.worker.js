...
export async function operationsWorkflowWorker(event) {
    ...
    try {
        switch (eventType) {
            case RealtimeEvent.QR_ASSISTANCE_REQUESTED: {
                const { tableId, sessionId, assistanceType, seatNumber } = payload;
                if (tableId && sessionId && assistanceType) {
                    await RestaurantOperationsService.handleQRAssistanceRequested(tenantId, outletId, tableId, sessionId, assistanceType, seatNumber);
                }
                break;
            }
            case RealtimeEvent.ORDER_STATUS_CHANGED: {
                ...
                if (order.orderStatus === "READY" && sessionId && tableId) {
                    await RestaurantOperationsService.handleOrderReady(tenantId, outletId, order._id, sessionId, tableId, seatNumber);
                }
                break;
            }
            case RealtimeEvent.TABLE_CLEANING_STARTED: {
                ...
                if (tableId) {
                    ...
                    if (session) {
                        await RestaurantOperationsService.handleTableCleaningStarted(tenantId, outletId, tableId, session._id);
                    }
                }
                break;
            }
            case RealtimeEvent.TABLE_STATUS_CHANGED: {
                ...
                if (tableId && status === "BILL_REQUESTED") {
                    ...
                    if (session) {
                        await RestaurantOperationsService.handleBillRequested(tenantId, outletId, tableId, session._id);
                    }
                }
                break;
            }
