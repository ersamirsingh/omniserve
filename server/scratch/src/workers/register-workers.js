...
async function compositeOrderCreatedWorker(event) {
...
async function compositeOrderStatusChangedWorker(event) {
    await statusSyncWorker(event);
    await realtimeSyncWorker(event);
...
async function compositeQRAssistanceRequestedWorker(event) {
    await operationsWorkflowWorker(event);
    await realtimeSyncWorker(event);
}
// Composite worker for Table Status Changes (e.g. check for BILL_REQUESTED, processes task spawning, then broadcasts)
async function compositeTableStatusChangedWorker(event) {
    await operationsWorkflowWorker(event);
    await realtimeSyncWorker(event);
}
// Composite worker for Table Cleaning Starts (spawns cleaning task, then broadcasts)
async function compositeTableCleaningStartedWorker(event) {
    await operationsWorkflowWorker(event);
    await realtimeSyncWorker(event);
}
export function initWorkerRegistry() {
