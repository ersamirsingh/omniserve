...
export async function inventorySyncWorker(event) {
    let connections = [];
    if (event.outletId) {
    ...
    for (const connection of connections) {
        ...
        const isCircuitOpen = await SyncEngineService.isCircuitOpen(event.tenantId, event.outletId, connection.provider);
        if (isCircuitOpen) {
        ...
        try {
            await connector.syncInventory(event.tenantId, event.outletId, connection._id, event, event.payload);
