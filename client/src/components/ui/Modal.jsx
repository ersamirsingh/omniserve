...
export default function Modal({ isOpen, onClose, title, children, size = 'md' }) {
  ...
  return (
    ...
    >
      <div 
        className={`relative w-full ${sizeClasses[size]} bg-white dark:bg-zinc-950 border border-border-base dark:border-zinc-800 rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto animate-scale-in p-0 my-auto mx-auto`} 
      ...
      >
        <div className="flex items-center justify-between px-6 py-5 border-b border-border-base dark:border-zinc-850">
          ...
          <button 
            type="button"
            className="flex items-center justify-center w-8 h-8 p-0 rounded-lg text-on-surface-variant dark:text-zinc-500 hover:bg-error/10 hover:text-error dark:hover:text-red-400 transition-all cursor-pointer border-none bg-transparent" 
            onClick={onClose}
          >
            <HiXMark size={20} />
          </button>
