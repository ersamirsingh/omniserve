...
export default function VariantsPage() {
  ...
  const fetchVariants = async (menuItemId) => {
    ...
    setLoading(true);
    try {
      const response = await listVariantsApi({ menuItemId });
  ...
  const fetchPage = async () => {
    ...
    try {
      ...
      if (initialMenuItemId) {
        const variantResponse = await listVariantsApi({ menuItemId: initialMenuItemId });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <h1 className="text-xl font-bold text-slate-100">Variants</h1>
        <Button onClick={openCreate} disabled={!menuItems.length}><HiPlus /> Add Variant</Button>
      </div>
      <div className="mb-5 max-w-md">
        <Select id="variant-filter-item" label="Menu Item" value={selectedMenuItemId} onChange={(e) => handleSelectedMenuItem(e.target.value)} disabled={!menuItems.length}>
