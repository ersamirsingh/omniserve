...
export default function AddonsPage() {
  ...
  const fetchData = async () => { setLoading(true); try { const r = await listAddonsApi(); setData(Array.isArray(r.data?.data) ? r.data.data : []); } catch { addToast('Failed', 'error'); } finally { setLoading(false); } };
  useEffect(() => { fetchData(); }, []);
  ...
  const handleSubmit = async (e) => { e.preventDefault(); try { const p = { ...form, price: Number(form.price) }; if (modal.mode === 'create') { await createAddonApi(p); addToast('Created', 'success'); } else { await updateAddonApi(modal.item._id, p); addToast('Updated', 'success'); } closeModal(); fetchData(); } catch (err) { addToast(err.response?.data?.message || 'Failed', 'error'); } };
  const handleDelete = async (id) => { if (!confirm('Delete?')) return; try { await deleteAddonApi(id); addToast('Deleted', 'success'); fetchData(); } catch { addToast('Failed', 'error'); } };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4"><h1 className="text-xl font-bold text-slate-100">Addons</h1><Button onClick={openCreate}><HiPlus /> Add Addon</Button></div>
      ...
      <Modal isOpen={modal.open} onClose={closeModal} title={modal.mode === 'create' ? 'New Addon' : 'Edit Addon'}>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <Input id="add-price" label="Price (₹)" type="number" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} required />
